...
function bfs(junctions: RouteMap) {
  let todo = [{ pos: keys(junctions)[0], score: 0, seen: new Set<string>() }];
  let maxScore = 0;
  while (todo.length > 0) {
    todo = todo.flatMap(({ pos, score, seen }) =>
      entries(junctions[pos] ?? ({} as RouteMap[string]))
        .filter(([p]) => !seen.has(p))
        .map(([p, dist]) => ({
          pos: p,
          score: score + dist,
